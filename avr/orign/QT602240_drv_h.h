...
*
* Info block struct. Since the size of object table is not known beforehand, 
* it's pointer to an array instead of an array. 
*
...
The host should ALWAYS use the CHG line as an indication that a message is available to read;
the host should NOT poll the device for messages or read the Message Processor object at any
other time. If the Message Processor object is read when the CHG line is not asserted, an
...
#define I2C_APPL_ADDR_0         0x4A    //0x4A,0x4B,0x24,0x25,
...
QT_EXT void CheckTouch( void);
QT_EXT void ReleaseChkTouch(void);

QT_EXT void write_message_to_usart(uint8_t msg[], uint8_t length);
...
unsigned short diagnostic_addr;
unsigned short diagnostic_size;
...
#define BAUD_38400  12          //RS232 波特率12=38.4KHz,25=19.2KHz
